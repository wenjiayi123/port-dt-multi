...
2026-09-12 功能巡检：修复训练参数、独立评测留痕、联动确认、旧数据残留、未知风险与图表导出，实际验证训练完成、暂停、恢复及取消。详见 [浏览器验收](docs/UI_BROWSER_AUDIT_20260912.md)和[自动化回归](docs/UI_FUNCTION_AUDIT_20260912.md)。岸电储能 V8 的 SAC/TD3 各 60,000 步诊断均未通过综合门，保留负指标及失败记录，没有晋级新冠军；见 [V8 训练结果](docs/SHORE_BESS_V8_RESULTS.md)。公开模型改用可核验的安全副本，权重不变，原模型身份通过映射保留；见[模型发布验证](docs/PUBLIC_MODEL_PRIVACY_20260912.md)。
