...
`public_port_ops_v1` 是为了复现接口、环境和测试而构造的集成数据集：公开输入包括新加坡海事及港务管理局发布的集装箱吞吐量，以及 NOAA CO-OPS 潮位预报；其余字段是有记录的确定性工程派生量。两类公开输入来自不同地理区域，因此它**不是**任何单一港口的实测时序，也不能用于单港绩效归因。

`public_port_ops_v1` is an integration dataset for reproducible adapters, environments, and tests. Its public inputs include container throughput published by the Maritime and Port Authority of Singapore and NOAA CO-OPS tide predictions; all remaining fields are documented deterministic engineering derivatives. Because the inputs span different geographies, it is **not** a measured single-port time series and must not be used for site-performance attribution.
...
同名数据集默认禁止覆盖，只有显式 `replace_existing=true` 才允许替换。字段映射、来源与派生边界见 [数据与接港契约](docs/DATASET_AND_PORT_ADAPTER.md) 和 [公开数据卡](docs/DATASET_CARD_public_port_ops_v1.md)。
...
默认启动只开放可信主链；旧工程界面模拟器需要显式开启，未审计的旧 RL 产物不随开源包分发。本机应用联动和生产执行也保持关闭。完整变量见 [.env.example](.env.example)。

The default runtime exposes the trusted core only. Legacy engineering UI simulators require explicit opt-in; unaudited legacy RL artifacts are not distributed. Local desktop launchers and production execution also remain disabled. See [.env.example](.env.example).
