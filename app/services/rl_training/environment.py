...
class PortOperationsEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        self._seed = int(seed)
        self._mins = np.nanmin(self.segment, axis=0)
        self._maxs = np.nanmax(self.segment, axis=0)
    ...
    def project_control(self, action: Any, *, soc: float, last_bess_kw: float) -> Dict[str, Any]:
        ...
        ramp = self.bess_power_kw * 0.35
        bess_kw = float(np.clip(requested_bess_kw, last_bess_kw - ramp, last_bess_kw + ramp))
        projected_soc = float(soc)
        if bess_kw >= 0:
            max_charge = (0.90 - projected_soc) * self.bess_capacity_kwh / (0.96 * self.step_hours)
            bess_kw = min(bess_kw, max(0.0, max_charge))
            projected_soc += bess_kw * self.step_hours * 0.96 / self.bess_capacity_kwh
        else:
            max_discharge = (projected_soc - 0.10) * self.bess_capacity_kwh * 0.96 / self.step_hours
    ...
    def step(self, action: Any):
        row = self._row()
        control = self._decode_action(action)
        projected = self.project_control(action, soc=self._soc, last_bess_kw=self._last_bess_kw)
        bess_kw = float(projected["bess_kw"])
        service_factor = float(projected["service_factor"])
        flex_command = float(projected["flexible_load_command"])
        ...
        exceed_kw = max(0.0, net_kw - self.demand_cap_kw)
