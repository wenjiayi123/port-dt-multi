...
BUSINESS_CAPABILITIES = [
    ...
    {"id": "quay", "name": "岸桥与装卸节拍", "state": "implemented", "engine": "V6 岸桥→水平运输→场桥资源链动作", "site_replacement": "QC 状态、作业路、Moves/h、故障码"},
    {"id": "yard", "name": "堆场与箱区流动", "state": "implemented", "engine": "堆场占用 + Yard Flow 动作", "site_replacement": "箱位、翻箱、堆存期、场桥任务"},
    {"id": "horizontal", "name": "集疏运 / AGV / 集卡", "state": "implemented", "engine": "V6 水平运输资源动作 + 专项 IQL 沙箱", "site_replacement": "车辆位置、任务队列、充电、路权"},
    {"id": "gate", "name": "闸口 / 铁路 / 驳船", "state": "implemented", "engine": "V6 闸口、铁路、驳船三路独立连续动作", "site_replacement": "预约、过闸、班列、驳船计划"},
    {"id": "energy", "name": "微电网与储能", "state": "implemented", "engine": "BESS、柔性负荷、峰值约束", "site_replacement": "电表、需量、电价、BMS、光储充"},
    {"id": "reefer", "name": "冷藏箱与岸电", "state": "implemented", "engine": "V6 冷藏保障和岸电独立动作 + 最低服务硬门", "site_replacement": "插拔箱、温控、岸电连接与费率"},
    {"id": "maintenance", "name": "设备健康与维护", "state": "implemented", "engine": "V6 维护资源动作 + 最低维护储备硬门", "site_replacement": "振动、温度、工单、备件、RUL 标签"},
    {"id": "regulatory", "name": "海关 / 监管查验与韧性", "state": "implemented", "engine": "V4/V6 查验延误状态、恢复动作与确定性放行边界", "site_replacement": "舱单、查验队列、放行事件、监管身份与回执"},
...
BUSINESS_DEPTH = {
    ...
    "regulatory": {
        "state_inputs": ["舱单/货类", "查验比例", "待放行队列", "监管服务能力", "放行事件时效"],
        "decision_outputs": ["查验资源恢复建议", "积压风险预警", "业务恢复优先级"],
        "hard_constraints": ["RL 不签发监管放行", "身份与回执必须来自主管系统", "危险品规则不可学习绕过"],
        "acceptance_metrics": ["监管延误箱时", "查验积压", "恢复时间", "错误放行率"],
    },
    "weather": {
...
BUSINESS_CODE_EVIDENCE = {
    ...
    "quay": ["app/services/rl_model/port_G_qc_mvp/rl_engine_g.py", "app/services/rl_training/coordinated_environment.py", "tests/test_rl_training.py"],
    "yard": ["app/services/rl_model/yard_crane/rl_engine_f.py", "app/services/rl_training/environment.py", "tests/test_v3.py"],
    "horizontal": ["app/services/rl_model/agv_charge/train_iql.py", "app/services/rl_training/coordinated_environment.py", "app/data_contracts/energyx_contract.json"],
    "gate": ["app/services/rl_training/coordinated_environment.py", "app/services/mobile_api/workflow.py", "docs/SITE_DATA_REPLACEMENT_CONTRACT_V3.md"],
    ...
    "reefer": ["app/data_contracts/energyx_contract.json", "app/services/rl_training/coordinated_environment.py", "app/services/rl_training/datasets.py"],
    "maintenance": ["app/services/monitoring_quality/data_quality.py", "app/services/monitoring_quality/alerts.py", "app/services/rl_training/coordinated_environment.py"],
    "regulatory": ["app/services/rl_training/regulatory_environment.py", "app/services/rl_training/coordinated_environment.py", "scripts/train_regulatory_resilience_v4.py"],
...
BUSINESS_EXECUTION_EVIDENCE = {
    "vessel": {
        ...
        "decision_source": "V3 selected SAC plus the V6 coordinated berth-priority action and hard safety projection",
        "runtime_endpoints": ["/api/rl/coordinated-business/evidence", "/api/v3/runtime/frame", "/api/v3/runtime/series?scenario=strategy"],
        "current_data_mode": "public-data calibrated replay; AIS/ETA and berth actuals pending",
        "model_output_available": True,
        "learned_optimizer_output": True,
    ...
    "quay": {
        "implementation_level": "model_backed_coordinated_offline",
        "implementation_label": "V6 协同优化器已实现·现场验证待接",
        "decision_source": "V6 coordinated SAC resource-chain action plus quay-crane RL sandbox and hard capacity projection",
        "runtime_endpoints": ["/api/rl/coordinated-business/evidence", "/api/curves/asset/qc-01?mode=forecast", "/api/forecast/qc-01"],
        "current_data_mode": "public aggregate/reanalysis plus declared engineering resource chain; no QC PLC feed",
        "model_output_available": True,
        "learned_optimizer_output": True,
    ...
    "horizontal": {
        "implementation_level": "model_backed_coordinated_offline",
        "implementation_label": "V6 协同优化器已实现·现场适配待接",
        "decision_source": "V6 horizontal-transport action in the quay-to-yard chain plus legacy AGV IQL sandbox",
        "runtime_endpoints": ["/api/rl/coordinated-business/evidence", "/api/rl/module_a/summary", "/api/rl/rollout/status"],
        "current_data_mode": "public aggregate/reanalysis and engineering fleet capacity; no fleet telemetry",
        "model_output_available": True,
        "learned_optimizer_output": True,
    ...
    "gate": {
        "implementation_level": "model_backed_coordinated_offline",
        "implementation_label": "V6 三通道独立优化·现场适配待接",
        "decision_source": "V6 SAC independently controls gate, rail and barge service intensities with queue/capacity state",
        "runtime_endpoints": ["/api/rl/coordinated-business/evidence", "/api/v3/runtime/frame"],
        "current_data_mode": "public aggregate/reanalysis with declared intermodal split; appointment and actual flows unavailable",
        "model_output_available": True,
        "learned_optimizer_output": True,
        "site_blockers": ["truck appointment/arrival/service/departure", "rail consist plan", "barge plan and customs release"],
        "fail_closed_fallback": "fixed-rule recommendation or 待接入港口; no lane, customs or dispatch authority",
    },
    "energy": {
        "implementation_level": "model_backed_specialized",
        "implementation_label": "专项模型已执行",
    ...
    "reefer": {
        "implementation_level": "model_backed_coordinated_offline",
        "implementation_label": "V6 保障动作已实现·温控现场源待接",
        "decision_source": "V6 reefer-support action with non-learnable minimum-service projection and energy coupling",
        "runtime_endpoints": ["/api/rl/coordinated-business/evidence", "/api/curves/asset/reefer-01?mode=forecast"],
        "current_data_mode": "public/engineering reefer demand scenario; plug and temperature telemetry unavailable",
        "model_output_available": True,
        "learned_optimizer_output": True,
        "site_blockers": ["container temperature/setpoint", "plug state", "cargo criticality and alarm acknowledgement"],
        "fail_closed_fallback": "minimum service is enforced outside RL; no container power command",
    },
    "maintenance": {
        "implementation_level": "model_backed_coordinated_offline",
        "implementation_label": "V6 资源建议已实现·工单执行仍由 CMMS",
        "decision_source": "V6 maintenance-resource action with non-learnable minimum reserve plus anomaly/drift monitoring",
        "runtime_endpoints": ["/api/rl/coordinated-business/evidence", "/api/v3/monitoring/evidence"],
        "current_data_mode": "engineering risk/backlog scenario; CMMS and condition telemetry pending",
        "model_output_available": True,
        "learned_optimizer_output": True,
        "site_blockers": ["condition sensors", "fault taxonomy", "CMMS work orders", "RUL labels and spare stock"],
        "fail_closed_fallback": "minimum maintenance reserve and conservative degradation; no work-order command",
    },
    "regulatory": {
        "implementation_level": "model_backed_resilience_non_authoritative",
        "implementation_label": "延误韧性建议已执行·放行权不属于 RL",
        "decision_source": "V4/V6 learned inspection-recovery recommendation; identities, legal rules and releases remain deterministic external authority",
        "runtime_endpoints": ["/api/rl/regulatory-resilience/evidence", "/api/rl/coordinated-business/evidence"],
        "current_data_mode": "official aggregate plus declared disruption scenario; no authorized manifest or release event feed",
        "model_output_available": True,
        "learned_optimizer_output": True,
        "site_blockers": ["manifest and inspection queue", "customs/authority release identities", "dangerous-goods rules", "signed release receipts"],
        "fail_closed_fallback": "RL may prioritize recovery advice only; it cannot create or bypass a regulatory release",
    },
    "weather": {
        ...
        "model_output_available": True,
        "learned_optimizer_output": False,
    ...
    "multiport": {
        ...
        "current_data_mode": "six public benchmark profiles; no authorized site adapter",
        "model_output_available": True,
        "learned_optimizer_output": False,
    ...
    payload = {
        ...
        "business_domain_coverage": {
            ...
            "no_independent_optimizer_count": sum(
                not bool(row.get("learned_optimizer_output"))
                for row in domain_depth.values()
            ),
            "learned_optimizer_output_count": sum(
                bool(row.get("learned_optimizer_output"))
                for row in domain_depth.values()
            ),
