...
def detect_anomalies(
    ...
    *,
    ...
    """
    统计法检测异常点（健壮版）：
    - 把非有限数（NaN/Inf）从统计量计算中剔除
    - Z分数 + IQR 联合判定
    - 对缺测/非数值：ignore_none=True 时视为“暂时有效”，避免把整个序列都打成异常
    """
    # 仅用“有限数值”参与统计（过滤 NaN/Inf）
    ...
    # 如果没有任何有限值，就全 True（交给插补）
    if not finite_vals:
        return [True] * n

    # Z 分数所需均值/方差（保证有限且非零）
    mu = statistics.fmean(finite_vals)
    sigma = statistics.pstdev(finite_vals)
    ...
    for _, v in series:
        if not (isinstance(v, (int, float)) and math.isfinite(v)):
            ...
            z = (v - mu) / sigma
            # Non-finite values are filtered before threshold comparison.
