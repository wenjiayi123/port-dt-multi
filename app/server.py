...
<main>
  ...
  <section class="card" style="grid-column:1 / -1;border-color:rgba(52,211,153,.42);">
    <div class="train-head">
      <div>
        <div class="train-title">固定业务KPI证据 <span class="en">Reproducible Business KPI Evidence</span></div>
        <div class="muted small">公开输入驱动的数字孪生反事实对照；显示精确值、对照组、留出区间和证据边界，不冒充港口实测KPI。</div>
      </div>
      <div id="businessEvidenceChip" class="progress-chip">正在校验证据</div>
    </div>
    <div class="train-stat-grid">
      <div class="stat-card"><span class="k">泊位利用率相对提升</span><span id="businessBerthKpi" class="v">—</span></div>
      <div class="stat-card"><span class="k">平均待泊时间缩短</span><span id="businessWaitKpi" class="v">—</span></div>
      <div class="stat-card"><span class="k">情景用电成本降低</span><span id="businessCostKpi" class="v">—</span></div>
      <div class="stat-card"><span class="k">最终留出记录</span><span id="businessHoldoutRows" class="v">—</span></div>
    </div>
    <div id="businessEvidenceDetail" class="train-summary small">等待读取 /api/rl/business-benchmark。</div>
  </section>
...
async function loadBusinessEvidence(){
  const response = await fetch("/api/rl/business-benchmark", {cache:"no-store"});
  if(!response.ok) throw new Error(await response.text());
  ...
  setText("#businessEvidenceDetail", `对照：静态FCFS + 固定能源时刻表；候选：多智能体编排 + 联合策略。时间切分 ${report.dataset?.split_sizes?.train||0}/${report.dataset?.split_sizes?.validation||0}/${report.dataset?.split_sizes?.test||0}，测试 ${period.start||"—"} → ${period.end||"—"}；数据与代码哈希已校验。口径：公开输入驱动仿真，非港口实测KPI，production_claim_allowed=false。`);
}
...
window.addEventListener("load", async ()=>{
  ...
  try{ await loadBusinessEvidence(); }catch(err){
    if($("#businessEvidenceChip")) $("#businessEvidenceChip").textContent = "证据校验失败 · BLOCKED";
    if($("#businessEvidenceDetail")) $("#businessEvidenceDetail").textContent = `业务KPI证据不可用：${String(err).slice(0,180)}`;
  }
...
async def rl_train_baselines(dataset_id: Optional[str] = Query(None)) -> JSONResponse:
    ...
    business = load_business_benchmark()
    business_dataset = business.get("dataset") or {}
    effective_dataset_id = (
        benchmark.get("dataset_id")
        or business_dataset.get("dataset_id")
    )
    is_business_dataset = (
        effective_dataset_id == business_dataset.get("dataset_id")
    )
    mobile_items = [
        {
            "id": item.get("id"),
            ...
            "library": item.get("implementation"),
            "action_space": item.get("action_space"),
            "trainable": item.get("trainable"),
    ...
    return JSONResponse(
        {
            **benchmark,
            ...
            "dataset": {
                ...
                "dataset_sha256": business_dataset.get("sha256")
                if is_business_dataset
                else None,
                "rows": business_dataset.get("rows")
                if is_business_dataset
                else None,
                "split_method": business_dataset.get("split_method")
                if is_business_dataset
                else None,
                "split_sizes": business_dataset.get("split_sizes")
                if is_business_dataset
                else None,
                "evidence_level": business.get("evidence_level")
                if is_business_dataset
                else "query_/api/rl/datasets/{id}/quality",
...
@app.get("/api/rl/business-benchmark", tags=["rl-panel"])
async def rl_business_benchmark() -> JSONResponse:
    try:
        report = await asyncio.to_thread(load_business_benchmark)
    ...
    return JSONResponse(
        {
            ...
            "dataset": {
                name: (report.get("dataset") or {}).get(name)
                for name in (
                    "dataset_id",
                    "sha256",
                    "rows",
            ...
            "coordinated_policy": report.get("coordinated_policy"),
...
async def rl_mobile_train_request_approve(
    ...
    requester = str(request.get("requested_by") or "").strip()
    if not operator:
        raise HTTPException(status_code=422, detail="审批人不能为空")
    if requester and operator == requester:
        raise HTTPException(
            status_code=409,
            detail="训练申请人与电脑端审批人必须为不同主体",
        )
...
_ENABLE_LEGACY_RL = os.getenv("PORT_DT_ENABLE_LEGACY_RL", "").strip().lower() in {"1", "true", "yes", "on"}
get_router = None
