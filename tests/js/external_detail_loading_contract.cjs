...
function setup(){
  ...
  const context={Date,JSON,Math,Number,String,Promise,console,setTimeout(fn,delay){timers.push({fn,delay});},document:{getElementById:node,createElement:()=>node('new-'+nodes.size)},window:{devicePixelRatio:1,addEventListener(){},__portDtLoadWhenActive(section,task){assert.equal(section,'ext-section');loaders.push(task);}},fetch(url){return new Promise((resolve,reject)=>requests.push({url,resolve,reject}));}};
  ...
  return{node,timers,requests,resolve,settle,context,loaders};
}
(async()=>{
  let h=setup();const first=h.node('btn-ext-provenance').click();
  h.loaders[0]();h.loaders[0]();
  assert.equal(h.requests.filter(x=>x.url.includes('external-signals')).length,1,'click plus repeated page-load starts coalesce');
  ...
  h.loaders[0]();assert.equal(h.requests.filter(x=>x.url.includes('external-signals')).length,2,'successful evidence is not reloaded by startup retry');
