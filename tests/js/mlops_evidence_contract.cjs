...
function setup(){
 ...
 const context={JSON,Number,String,Promise,console,setTimeout(fn,delay){timers.push({fn,delay});},document:{getElementById:node},fetch:url=>new Promise((resolve,reject)=>requests.push({url,resolve,reject}))};vm.createContext(context);vm.runInContext(script,context);return{node,timers,requests,context};
}
...
(async()=>{
 let h=setup();const detail=h.node('btn-mlops-registry').click();h.timers.forEach(x=>x.fn());assert.equal(h.requests.length,1);h.requests[0].resolve({ok:true,json:async()=>data});await detail;
 assert.equal(JSON.parse(h.node('mlops-detail').textContent).summary.formal_runs,34);h.timers.forEach(x=>x.fn());assert.equal(h.requests.length,1,'startup retry does not reset selected detail');assert.match(h.node('dq-res').textContent,/通过/);assert.equal(h.node('o11y-job').value,'fixture-a, fixture-b');assert.equal(h.node('dq-dataset').value,'fixture-dataset');
