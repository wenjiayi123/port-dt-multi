...
DQN 使用储能、服务强度和柔性负荷的有限动作格点；其余 RL 算法使用相同控制量的连续版本。MPC 在相同环境上进行滚动时域约束优化，不训练神经网络。

## 防止数据泄漏

`PortDataset.split()` 只做时间顺序切分。默认前 80% 是训练集，后 20% 是测试留出集：
...
权重会归一化。环境同时约束储能 SOC、功率、爬坡速度和服务强度。当前参数是工程初值，接港时必须按设备铭牌、EMS/BMS 限值、合同需量和作业 SLA 校准。
