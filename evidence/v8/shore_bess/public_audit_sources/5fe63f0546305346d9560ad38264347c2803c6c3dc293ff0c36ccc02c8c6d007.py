...
def equivalence():
 ...
 torch.set_num_threads(1);model=TD3.load(path,device='cpu');result=[];comparisons=0;failures=[];expected_differences=[]
 for start in row['evaluation']['starts']:
  ...
  for step in range(168):
   ...
   flex_operand_scales=[max(1.0,env._row_context()['auxiliary_shore_kw']*env.flex_limit,env._flex_backlog_kwh) for env in environments]
   outputs=[env.step(act) for env,act in zip(environments,predictions)]
   main_map,candidate_map=(v[4]['action_mapping'] for v in outputs)
   if {k:v for k,v in main_map.items() if k!='version'}!={k:v for k,v in candidate_map.items() if k!='version'}:
    changed=[k for k in main_map if k!='version' and main_map[k]!=candidate_map[k]]
    lo,hi=candidate_map['flex_interval_kw']
    allowed=(set(changed)<={'flex_interval_kw','mapped_action'} and main_map['mapped_action'][0]==candidate_map['mapped_action'][0]
             and 0<lo-hi<=32*np.finfo(np.float64).eps*max(*flex_operand_scales,abs(lo),abs(hi)))
    (expected_differences if allowed else failures).append({'start':start,'step':step,'kind':'flex_roundoff_mapping_reconciliation' if allowed else 'unexplained_mapping_difference',
      'candidate':candidate_map,'main':main_map,'candidate_interval_reversal_kw':lo-hi})
   main_ledger,candidate_ledger=(env.totals for env in environments)
   changed={k:[main_ledger[k],v] for k,v in candidate_ledger.items() if main_ledger[k]!=v}
   if changed:
    allowed=set(changed)<={'reward','projection_count','projection_rate'}
    (expected_differences if allowed else failures).append({'start':start,'step':step,'kind':'legacy_projection_diagnostic_change' if allowed else 'unexplained_ledger_difference','fields':changed})
   for name,values in [('reward',[v[1] for v in outputs]),('termination',[(v[2],v[3]) for v in outputs]),
                        ('final_action',[v[4]['final_action'] for v in outputs]),
 ...
 return {'status':'PASS' if not failures else 'FAIL','checkpoint_path':str(path.relative_to(ROOT)),
         ...
         'compared_environment_steps':comparisons,'comparison':'exact equality of float32 observation, neural action, executed action, V8 learning reward, termination, business and safety ledger. Expected flex-roundoff normalization and inherited V3 projection diagnostic changes are separately checked and retained.',
         'mapping_version_labels':{'main':ShoreBESSV8SACEnv.action_mapping_version,'candidate':candidate.action_mapping_version},
         'unexplained_mismatch_count':len(failures),'mismatches':failures,'expected_differences':expected_differences,'per_window':result}
