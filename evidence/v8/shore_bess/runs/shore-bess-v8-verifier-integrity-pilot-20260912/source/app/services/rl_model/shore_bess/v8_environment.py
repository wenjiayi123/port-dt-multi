...
class ShoreBESSV8SACEnv(ShoreBESSV8Env):
    ...
    def _continuous_bounds(self, ctx):
        ...
        if remaining > 0:
            ...
            charge_room = max(0.0, (min(self.soc_max, self.soc_initial + next_band) - self._soc) * self.energy_kwh / self.charge_eff)
            discharge_room = max(0.0, (self._soc - max(self.soc_min, self.soc_initial - next_band)) * self.energy_kwh * self.discharge_eff)
            def with_braking(room):
                return room if room <= self.ramp_kw else (room + self.ramp_kw) / 2.0
            lower = max(lower, -with_braking(charge_room))
            upper = min(upper, with_braking(discharge_room))
        return lower, upper
    ...
    def step(self, action):
        ...
        mapped = np.asarray([bess / self.power_kw, flex / max(capacity, 1e-9)], dtype=np.float32)
