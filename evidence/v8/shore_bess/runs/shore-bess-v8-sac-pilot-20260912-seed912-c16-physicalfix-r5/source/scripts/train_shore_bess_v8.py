...
class TrainingEpisodeLedger:
    """Persist completed physical training episodes before the next optimizer update."""
    phase = "after_environment_step_before_replay_storage_and_optimizer"
    ...
    def record(self, model: Any, infos: list[dict]) -> None:
        for info in infos:
            if "episode_metrics" not in info:
                continue
            metrics = {key: float(value) for key, value in info["episode_metrics"].items()
                       if isinstance(value, (int, float, np.number))}
            if not metrics or not all(np.isfinite(value) for value in metrics.values()):
                raise ValueError("completed training episode requires finite physical ledger metrics")
            names = sorted(metrics)
            if self.metric_names is not None and names != self.metric_names:
                raise ValueError("physical training episode metric schema changed during the run")
            self.metric_names = names
            row = {"episode_index": self.episodes + 1,
                   "environment_steps": int(model.num_timesteps),
                   "optimizer_step_calls_at_observation": int(model._v8_optimizer_step_calls),
                   "sb3_update_counter_at_observation": int(model._n_updates),
                   "record_phase": self.phase,
                   **{"episode_metrics." + key: metrics[key] for key in names}}
    ...
    def description(self) -> dict[str, Any]:
        self.handle.flush()
        return {"path": relative(self.path), "sha256": file_sha256(self.path),
                "completed_episodes": self.episodes, "record_phase": self.phase,
                "metrics_source": "unaltered numeric environment info.episode_metrics from physical training rollouts",
                "partial_final_episode_included": False,
                "claim_boundary": "Physical training episode ledger, not validation or a post-optimizer checkpoint."}
...
def replay_configuration(algorithm: str, n_step: int) -> dict[str, Any]:
    if not 1 <= n_step <= EPISODE_HOURS:
        raise ValueError("n-step must be between 1 and the episode horizon")
    if n_step > 1 and algorithm != "td3":
        raise ValueError("multi-step replay is supported only for the gamma=1 TD3 ablation")
    return {
        "class": ("app.services.rl_model.shore_bess.v8_replay.NStepReplayBuffer" if n_step > 1
                  else "stable_baselines3.common.buffers.ReplayBuffer" if algorithm in {"dqn", "sac", "td3"}
                  else "on_policy_rollout_buffer"),
        "kwargs": {"n_step": n_step, "gamma": 1.0} if n_step > 1 else {},
        "approximation": {
            "off_policy_importance_correction": False,
            "behavior_policy_intermediate_rewards": n_step > 1,
            ...
            "vanilla_algorithm_equivalence_claimed": n_step == 1,
            "boundary": ("Experimental off-policy multi-step TD3; no importance correction. Not unchanged vanilla TD3."
                         if n_step > 1 else "Original one-step replay or on-policy rollout collection."),
        },
    }


def replay_construction_kwargs(config: dict[str, Any]) -> dict[str, Any]:
    if config.get("n_step", 1) == 1:
        return {}
    from app.services.rl_model.shore_bess.v8_replay import NStepReplayBuffer
    if config["algorithm"] != "stable_baselines3.TD3" or config["gamma"] != 1.0:
        raise ValueError("multi-step replay requires TD3 with gamma=1")
    expected = replay_configuration("td3", config["n_step"])
    if config["replay_buffer_config"] != expected:
        raise ValueError("multi-step replay configuration differs from the declared variant")
    return {"replay_buffer_class": NStepReplayBuffer, "replay_buffer_kwargs": dict(expected["kwargs"])}


def capture_replay_snapshot(model: Any, phase: str, *, flush_at_training_end: bool = False) -> dict | None:
    """Freeze replay evidence into the model ZIP; a final flush performs no learning."""
    buffer = getattr(model, "replay_buffer", None)
    if not hasattr(buffer, "describe"):
        return None
    counters_before = {"optimizer_step_calls": int(model._v8_optimizer_step_calls),
                       "sb3_update_counter": int(model._n_updates),
                       "optimizer_step_calls_by_component": dict(model._v8_optimizer_steps_by_component)}
    emitted = buffer.flush_pending(reason="training_budget_end") if flush_at_training_end else None
    counters_after = {"optimizer_step_calls": int(model._v8_optimizer_step_calls),
                      "sb3_update_counter": int(model._n_updates),
                      "optimizer_step_calls_by_component": dict(model._v8_optimizer_steps_by_component)}
    if counters_after != counters_before:
        raise ValueError("replay storage-only flush unexpectedly changed optimizer counters")
    snapshot = buffer.describe()
    if not snapshot["transition_accounting_balanced"] or not snapshot["emitted_horizon_accounting_balanced"]:
        raise ValueError("multi-step replay transition accounting does not balance")
    snapshot["checkpoint_phase"] = phase
    if flush_at_training_end:
        snapshot.update(optimizer_steps_before_flush=counters_before, optimizer_steps_after_flush=counters_after,
                        storage_only_flush_emitted=int(emitted))
...
def model_parameters(model: Any) -> dict[str, Any]:
    ...
    if hasattr(model, "_v8_replay_snapshot"):
        result["replay_buffer_class"] = model.replay_buffer_class.__module__ + "." + model.replay_buffer_class.__name__
        result["replay_buffer_kwargs"] = copy.deepcopy(model.replay_buffer_kwargs)
        result["replay_buffer_snapshot"] = copy.deepcopy(model._v8_replay_snapshot)
    return result
...
def main() -> None:
    ...
    parser.add_argument("--n-step", type=int, default=1, help="TD3-only off-policy multi-step ablation; default preserves one-step training")
    parser.add_argument("--run-id")
    parser.add_argument("--pilot", action="store_true")
    args = parser.parse_args()
    try:
        replay_config = replay_configuration(args.algorithm, args.n_step)
    ...
    if args.n_step > 1:
        source_paths.append(ROOT / "app/services/rl_model/shore_bess/v8_replay.py")
    source_hashes = {}
    ...
    run_config = {"algorithm": algorithm_name, "seeds": seeds,
                  "n_step": args.n_step, "replay_buffer_config": replay_config,
                  "algorithm_variant": (f"td3_n_step_{args.n_step}_off_policy_uncorrected" if args.n_step > 1
                                        else "vanilla_" + args.algorithm),
    ...
    manifest = {"schema": "port-shore-bess-v8-manifest.v1", "run_id": run_id, "started_at": utc_now(),
                ...
                "access_protocol_details": {
                    "historical_csv_bytes": "Complete 2024-2025 CSV physically loaded; full-table quality checks performed.",
                    "historical_test_partition": "August-December 2025 rows excluded from gradients, normalization and model selection; evaluated only after formal selection.",
                    "pilot_test_evaluation_performed": False if args.pilot else None,
                    "forward_csv_bytes": "Separate 2026 CSV is not loaded in pilots; formal evaluation loads it after selection freeze.",
                    "test_status_not_opened_meaning": "No test-partition policy evaluation; does not claim historical CSV bytes were never read.",
                },
                "teacher_actions_used": False, "warm_start_used": False,
                "training_episode_ledger_contract": TrainingEpisodeLedger.phase,
    ...
    for seed in seeds:
        ...
        construction_kwargs.update(replay_construction_kwargs(run_config))
        ...
        capture_replay_snapshot(model, "initial_zero_updates")
        optimizer_hooks = []
        ...
        curve: list[dict] = []
        episode_ledger = TrainingEpisodeLedger(seed_dir / "training_episodes.csv")

        def checkpoint(phase: str = "post_rollout_and_optimizer", *, flush_at_training_end: bool = False) -> None:
            path = seed_dir / f"step_{int(model.num_timesteps)}.zip"
            if path.exists():
                raise FileExistsError("immutable checkpoint already exists")
            capture_replay_snapshot(model, phase, flush_at_training_end=flush_at_training_end)
            model.save(path)
        ...
        class Checkpoints(BaseCallback):
            ...
            def _on_training_end(self):
                checkpoint("training_end_after_storage_only_flush" if args.n_step > 1 else "post_rollout_and_optimizer",
                           flush_at_training_end=args.n_step > 1)
