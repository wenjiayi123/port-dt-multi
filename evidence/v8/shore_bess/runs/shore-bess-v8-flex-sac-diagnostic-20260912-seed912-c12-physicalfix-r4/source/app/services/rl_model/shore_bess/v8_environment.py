...
class ShoreBESSV8Env(ShoreBESSEnv):
    ...
    def _project(self, action, ctx):
        # The historical V3 step casts its argument to float32. Keep that
        # archived implementation unchanged, but use V8's exact physical
        # command here so a sub-watt rounding error cannot trigger a safety
        # penalty or break an exactly reachable braking boundary.
        exact = getattr(self, "_exact_projection_action", None)
        bounded = np.clip(np.asarray(action if exact is None else exact, dtype=np.float64), -1, 1)
    ...
    def step(self, action):
        if self.discrete:
            ...
            raw = LATTICE[int(value)].astype(np.float64)
        else:
        ...
        timestep = self._step
        self._exact_projection_action = raw.copy()
        try:
            obs, _, terminated, truncated, info = super().step(raw)
        finally:
            self._exact_projection_action = None
...
class ShoreBESSV8SACEnv(ShoreBESSV8Env):
    ...
    """
    action_deadband = 0.1
    action_mapping_version = "physical_feasible_interval_float64_bidirectional_braking_v2"
    ...
    def _continuous_bounds(self, ctx):
        ...
        if remaining > 0:
            ...
            roundtrip = self.charge_eff * self.discharge_eff
            # Below one ramp step, the next action can cross zero. Stopping
            # at zero is unnecessarily restrictive under a shrinking SOC
            # envelope: e.g. charge 340 kW now, discharge 54 kW next hour.
            charge_braking = ((charge_room + self.ramp_kw) / 2 if charge_room > self.ramp_kw
                              else (roundtrip * charge_room + self.ramp_kw) / (1 + roundtrip))
            discharge_braking = ((discharge_room + self.ramp_kw) / 2 if discharge_room > self.ramp_kw
                                 else (discharge_room + roundtrip * self.ramp_kw) / (1 + roundtrip))
            lower = max(lower, -charge_braking)
            upper = min(upper, discharge_braking)
        return self._roundoff_interval(lower, upper, self.power_kw,
                                      self.energy_kwh / self.charge_eff,
                                      self.energy_kwh * self.discharge_eff)

    @staticmethod
    def _roundoff_interval(lower, upper, *scales):
        # Propagate precision from the operands (including SOC-to-kW energy
        # conversion), not just endpoints that may nearly cancel to zero.
        roundoff = 32 * np.finfo(np.float64).eps * max(1.0, abs(lower), abs(upper), *scales)
        if 0 < lower - upper <= roundoff:
            # Merge mathematically equal endpoints only at machine precision.
            # The independent physical-power tolerance remains 1e-6 kW.
    ...
    def step(self, action):
        ...
        lower, upper = self._roundoff_interval(lower, upper, self.power_kw,
                                              self.energy_kwh / self.charge_eff,
                                              self.energy_kwh * self.discharge_eff)
        ...
        flex_low, flex_high = self._roundoff_interval(flex_low, flex_high, capacity, self._flex_backlog_kwh)
