...
class BESSEnergyV3Env(gym.Env):
    ...
    def _project(self, action: np.ndarray, ctx: Mapping[str, float]) -> Dict[str, Any]:
        ...
        if power < 0.0 and net_kw > self.soft_cap_kw:
            capped_charge = max(0.0, self.soft_cap_kw - float(ctx["base_load_kw"]))
            # A soft demand-price target must not invalidate terminal energy
            # recovery. The old second projection left SOC below reachable_low.
            minimum_recovery = max(0.0, (reachable_low - self._soc) * self.energy_kwh / self.charge_eff)
            power = -max(capped_charge, minimum_recovery)
            next_soc = self._soc + (-power) * self.charge_eff / self.energy_kwh
            net_kw = float(ctx["base_load_kw"]) - power
            reasons.append("terminal_recovery_before_soft_peak_target" if minimum_recovery > capped_charge else "demand_charge_no_new_peak")
        reserve_cap = max(0.0, min(available_power - max(0.0, power), max_discharge - max(0.0, power)))
        reserve = min(reserve, reserve_cap)
    ...
    def step(self, action: Any):
        ...
        if terminated:
            ...
            self._totals.update({
                ...
                "terminal_soc_absolute_error": abs(self._soc - self.soc_initial),
                "terminal_energy_debt_kwh": max(0.0, self.soc_initial - self._soc) * self.energy_kwh / self.charge_eff,
            })
