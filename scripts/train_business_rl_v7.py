...
def gates(name, evaluation, reference, config):
    ...
    for metric in ("guardrail_violation_rate", "terminal_soc_error", "terminal_soc_absolute_error", "terminal_energy_debt_kwh", "terminal_flex_backlog_kwh", "shore_sla_violation_kwh"):
        if metric in m:
...
def improves_incumbent(delta):
    """Require reliable added value before replacing an existing RL champion."""
    return (delta["total_cost_cny"]["ci_low"] > 0
            and delta["carbon_kg"]["ci_low"] >= -0.01
            and delta["peak_kw"]["ci_low"] >= -0.01)
...
def main():
    ...
    if args.block <= 0 or len(set(seeds)) != len(seeds) or args.steps < 3 * args.block:
    ...
    if any(not np.isfinite(v) or v < 0 for v in (args.entropy, args.carbon_multiplier, args.peak_multiplier)) or not np.isfinite(args.learning_rate) or args.learning_rate <= 0:
        parser.error("learning rate must be positive; finite nonnegative reward weights and entropy are required")
    ...
    promotion = {"candidate_qualified": all(checks.values()), "incumbent_comparison": None, "promoted": all(checks.values())}
    champion_pointer = root / "offline_champion.json"
    if champion_pointer.exists() and all(checks.values()):
        incumbent = json.loads(champion_pointer.read_text())
        incumbent_report_path = ROOT / incumbent["report_path"]
        if file_sha256(incumbent_report_path) != incumbent["report_sha256"]:
            raise ValueError("incumbent evidence hash mismatch")
        incumbent_model_path = resolve_model_artifact(ROOT, incumbent["model_path"], incumbent["model_sha256"])
        incumbent_report = json.loads(incumbent_report_path.read_text())
        incumbent_config = incumbent_report["config"]
        incumbent_cls = SAC if incumbent_config["algorithm"] == "stable_baselines3.SAC" else DQN
        incumbent_model = incumbent_cls.load(incumbent_model_path, device="cpu")
        incumbent_lattice = incumbent_config.get("action_lattice")
        def incumbent_predict(obs, env):
            a = incumbent_model.predict(obs, deterministic=True)[0]
            return np.asarray(incumbent_lattice[int(a)], dtype=np.float32) if incumbent_lattice else a
        incumbent_eval = evaluate(module, factory(test), incumbent_predict, test_starts)
        delta = compare(selected["test"]["evaluation"], incumbent_eval)
        promotion.update({"incumbent_comparison": delta, "incumbent_model_sha256": incumbent["model_sha256"], "promoted": improves_incumbent(delta)})
